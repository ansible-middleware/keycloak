  ...
  authorization_settings:
    description:
      - A data structure defining the authorization settings for this client. For reference, please see the Keycloak API docs
        at U(https://www.keycloak.org/docs-api/latest/rest-api/index.html#_resourceserverrepresentation). This is C(authorizationSettings)
        in the Keycloak REST API.
    type: dict
    aliases:
...
CLIENT_COMPARE_EXCLUDE = CLIENT_META_DATA + ["defaultRoles"]
EMPTY_OPTIONAL_URL_FIELDS = ("rootUrl", "adminUrl", "baseUrl")
LIST_CLIENT_PARAMS = frozenset(
    {
        "redirect_uris",
        "web_origins",
        "default_roles",
...
def _sorted_str_list(clientrep, field):
    if field in clientrep:
        clientrep[field] = sorted(_coerce_str_list(clientrep[field]))
    else:
        clientrep[field] = []
...
def normalise_cr(clientrep, remove_ids=False):
    ...
    for url_field in EMPTY_OPTIONAL_URL_FIELDS:
        if clientrep.get(url_field) == "":
            clientrep.pop(url_field, None)

    _sorted_str_list(clientrep, "defaultClientScopes")
    _sorted_str_list(clientrep, "optionalClientScopes")
    _sorted_str_list(clientrep, "redirectUris")
    _sorted_str_list(clientrep, "defaultRoles")
    ...
    _sorted_str_list(clientrep, "webOrigins")

    return clientrep


def client_rep_is_unchanged(before_client, desired_client):
    before_norm = normalise_cr(before_client, remove_ids=True)
    desired_norm = normalise_cr(desired_client, remove_ids=True)
    return is_struct_included(desired_norm, before_norm, CLIENT_COMPARE_EXCLUDE)
...
def main():
    ...
    for client_param in client_params:
        ...
        if client_param == "protocol_mappers":
            ...
            continue
        elif client_param in LIST_CLIENT_PARAMS and isinstance(new_param_value, str):
            new_param_value = [new_param_value]
        elif client_param in ("root_url", "admin_url", "base_url") and new_param_value == "":
            continue
    ...
    if not before_client:
        ...
        if state == "present":
            ...
            # no changes
            if client_rep_is_unchanged(before_client_with_scopes, desired_client_with_scopes):
                result["changed"] = False
